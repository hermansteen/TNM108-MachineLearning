...
#shuffle dataset before cross validation
...
best_features = 4
rfe_lin = RFE(estimator=LinearRegression(), n_features_to_select=4).fit(X, Y)
...
rfe_lin = RFE(estimator=LinearRegression(), n_features_to_select=best_features).fit(X, Y)
